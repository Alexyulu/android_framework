            ...
            public Flowable<T> apply(Flowable<BaseResponse<T>> httpResponseFlowable) {
                return httpResponseFlowable.flatMap(new Function<BaseResponse<T>, Flowable<T>>() {
                    ...
                    public Flowable<T> apply(BaseResponse<T> tBaseResponse) {
                        BaseResponse.HeaderBean header = tBaseResponse.getHeader();
                        if (header == null) {
                            return Flowable.error(new ApiException("服务器返回异常"));
                        } else if(header.getCode() == null) {
                            return createData(tBaseResponse.getBody());
                        } else {
                            return Flowable.error(new ApiException(header.getMsg(), header.getCode()));
                        }
                    }
                });
            }
